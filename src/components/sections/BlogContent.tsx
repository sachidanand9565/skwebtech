  ...
  return (
    <>
      ...
      <section className="relative pt-24 md:pt-28 pb-10 bg-void overflow-hidden">
        ...
        <div className="container-custom relative z-10 text-center">
          ...
          <motion.h1
            ...
            className="text-4xl sm:text-5xl font-heading font-bold text-slate-900 mb-5 leading-tight"
          >
          ...
          >
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-10 py-3.5 rounded-full bg-white backdrop-blur-sm border border-slate-200 text-slate-900 placeholder-slate-500
                           focus:outline-none focus:border-primary-500/50 focus:bg-white focus:ring-2 focus:ring-primary-500/20 transition-all text-sm"
              />
              {searchQuery && (
                <button onClick={() => setSearchQuery('')} className="absolute right-4 top-1/2 -translate-y-1/2 text-slate-500 hover:text-slate-900 transition-colors">
                  <X size={16} />
                </button>
              )}
            </div>
          ...
          >
            <button
              ...
              className={`relative px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${
                activeCategory === 'all'
                  ? 'text-void'
                  : 'bg-white text-slate-700 hover:bg-white/[0.1] border border-slate-200'
              }`}
            >
              {activeCategory === 'all' && (
                <motion.span
          ...
          {(activeCategory !== 'all' || searchQuery) && (
            <div className="flex items-center justify-center gap-2 mt-4">
              <span className="text-slate-600 text-sm">{filteredPosts.length} result{filteredPosts.length !== 1 ? 's' : ''}</span>
              <button onClick={() => { setActiveCategory('all'); setSearchQuery(''); }} className="text-primary-400 text-sm hover:text-primary-600 underline underline-offset-2">
                Clear filters
              </button>
            </div>
          )}
      ...
      {filteredPosts.length === 0 && (
        <section className="py-12 bg-void">
          <div className="container-custom text-center">
            <div className="w-20 h-20 bg-white border border-slate-200 rounded-full flex items-center justify-center mx-auto mb-6">
              <Search size={32} className="text-slate-500" />
            </div>
            <h2 className="text-2xl font-heading font-bold text-slate-900 mb-3">No articles found</h2>
            <p className="text-slate-600 mb-6">Try adjusting your search or browse all posts.</p>
      ...
      <section className="relative py-10 bg-void overflow-hidden">
        <div className="container-custom">
          <div className="max-w-2xl mx-auto relative overflow-hidden rounded-3xl border border-slate-200 bg-void-100/80 p-10 text-center border-glow">
            ...
            <div className="relative z-10">
              <h2 className="text-2xl font-heading font-bold text-slate-900 mb-3">Subscribe to Our Newsletter</h2>
              <p className="text-slate-600 text-sm mb-7">Get the latest articles, tips, and insights delivered to your inbox. No spam, unsubscribe anytime.</p>
              <form className="flex flex-col sm:flex-row gap-3 max-w-sm mx-auto">
                <input
                  type="email"
                  placeholder="Your email address"
                  className="flex-1 px-4 py-3 rounded-full bg-white border border-slate-200 text-slate-900 placeholder-slate-500 focus:outline-none focus:border-primary-500/50 focus:ring-2 focus:ring-primary-500/20 text-sm transition-all"
                />
