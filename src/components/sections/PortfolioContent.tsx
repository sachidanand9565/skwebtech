...
export default function PortfolioContent({ initialProjects }: { initialProjects: Project[] }) {
  ...
  return (
    <section className="relative py-10 bg-void-50 overflow-hidden">
      ...
      <div className="container-custom relative z-10">
          ...
          <div className="max-w-md mx-auto mb-6">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-10 py-3 rounded-full bg-white border border-slate-200 text-slate-900 placeholder-slate-500
                           focus:bg-white focus:border-primary-500/50 focus:ring-2 focus:ring-primary-500/20
        ...
        {filteredProjects.length > 0 ? (
          ...
          <div className="text-center py-20">
            <div className="w-16 h-16 bg-white border border-slate-200 rounded-2xl flex items-center justify-center mx-auto mb-5">
              <Search size={28} className="text-slate-500" />
            </div>
            <h2 className="text-xl font-heading font-semibold text-slate-900 mb-2">No Projects Found</h2>
...
function ProjectCard({ project, index }: { project: Project; index: number }) {
  ...
  return (
    <div className="group h-full flex flex-col rounded-2xl overflow-hidden border border-slate-200 bg-white backdrop-blur-sm
      ...
      <div className={`relative h-52 overflow-hidden bg-gradient-to-br ${gradient}`}>
        <div className="absolute inset-0 flex items-center justify-center">
          <span className="text-5xl font-heading font-bold text-slate-600">{project.title.charAt(0)}</span>
        </div>
        {project.image && (
          <Image
            src={project.image}
        ...
        <div className="absolute top-3 left-3">
          <span className="px-2.5 py-1 bg-void/70 backdrop-blur-md border border-slate-300 rounded-full text-xs font-medium text-slate-900 shadow-sm">
      ...
      <div className="p-6 flex flex-col flex-1">
        <div className="flex flex-wrap gap-1.5 mb-4">
          {project.technologies.slice(0, 4).map((tech) => (
            <span key={tech} className="px-2.5 py-1 bg-primary-500/[0.08] border border-primary-500/15 text-primary-600 rounded-lg text-xs font-medium">
              {tech}
            </span>
          ))}
        </div>
        <h3 className="text-lg font-heading font-bold text-slate-900 mb-2 group-hover:text-primary-600 transition-colors line-clamp-1">
          {project.title}
        </h3>
