...
export default function CTASection({
  ...
  return (
    <section className="relative py-12 md:py-16 overflow-hidden bg-void">
      {/* Layered ambience */}
      <div className="glow-orb top-0 left-1/4 w-[520px] h-[520px] bg-primary-500/[0.1] -translate-y-1/2" />
      <div className="glow-orb bottom-0 right-1/4 w-96 h-96 bg-secondary-500/[0.12] translate-y-1/3" style={{ animationDelay: '5s' }} />
      ...
      <div className="container-custom relative z-10">
        <div className="max-w-3xl mx-auto text-center">
          ...
          {/* Star rating badge */}
          <Reveal y={16}>
            <div className="inline-flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-full mb-8 backdrop-blur-sm">
              <div className="flex gap-0.5">
                {[...Array(5)].map((_, i) => (
                  <Star key={i} size={13} className="text-amber-400 fill-amber-400" />
                ))}
              </div>
              <span className="text-slate-700 text-sm">Trusted by 100+ businesses across India</span>
            </div>
          </Reveal>

          {/* Title */}
          <Reveal delay={0.1}>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-heading font-bold text-slate-900 mb-5 leading-tight text-balance">
              {title.includes('?') ? (
                <>
                  {title.split('?')[0]}
