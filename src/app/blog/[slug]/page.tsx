      ...
      <article>
        ...
        <header className="relative pt-28 md:pt-36 pb-14 bg-void overflow-hidden">
          ...
          <div className="container-custom relative z-10">
            <Link href="/blog" className="inline-flex items-center gap-2 text-slate-600 hover:text-slate-900 text-sm font-medium mb-8 transition-colors group">
            ...
            <div className="max-w-3xl">
              <Reveal y={16}>
                <div className="flex items-center gap-3 mb-5">
                  <span className="px-3 py-1.5 bg-primary-500/10 border border-primary-500/25 text-primary-600 text-xs font-semibold rounded-full">
                    {post.category}
                  </span>
              ...
              <Reveal delay={0.1}>
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-heading font-bold text-slate-900 mb-5 leading-tight">
                  {post.title}
                </h1>
              </Reveal>
              <Reveal delay={0.2}>
              ...
              <Reveal delay={0.3}>
                <div className="flex flex-wrap items-center gap-6">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-500/30 to-secondary-500/30 border border-slate-200 flex items-center justify-center">
                      <span className="text-primary-600 font-bold text-sm">{post.author.name.charAt(0)}</span>
                    </div>
                    <div>
                      <div className="font-medium text-slate-900 text-sm">{post.author.name}</div>
        ...
        <div className="container-custom -mt-6 relative z-10">
          <Reveal delay={0.15} y={36}>
            <div className="relative aspect-[21/9] rounded-2xl overflow-hidden border border-slate-200 shadow-soft-lg">
              ...
              <div className="lg:col-span-8">
                <div
                  className="prose prose-lg max-w-none
                    ...
                    prose-blockquote:border-primary-500 prose-blockquote:text-slate-700 prose-blockquote:bg-primary-500/[0.06] prose-blockquote:rounded-r-xl prose-blockquote:py-1
                    prose-code:bg-white prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded prose-code:text-primary-600
                    prose-pre:bg-void-100 prose-pre:border prose-pre:border-slate-200 prose-pre:rounded-2xl
                    prose-hr:border-slate-200 prose-img:rounded-2xl"
                ...
                <div className="mt-12 pt-8 border-t border-slate-200">
                  <div className="flex items-center flex-wrap gap-2">
                    ...
                    {post.tags.map((tag) => (
                      <span key={tag} className="px-3 py-1.5 bg-white border border-slate-200 text-slate-700 text-sm rounded-full hover:border-primary-500/40 hover:text-primary-600 transition-colors cursor-pointer">
                        {tag}
                      </span>
                    ))}
                  </div>
                    ...
                    <div className="flex gap-2">
                      {[
                        ...
                        <a key={label} href={href} target="_blank" rel="noopener noreferrer" aria-label={`Share on ${label}`}
                          className={`w-9 h-9 bg-white border border-slate-200 rounded-xl flex items-center justify-center text-slate-600 ${hover} hover:text-slate-900 hover:border-transparent transition-all`}>
                          <Icon size={16} />
                        </a>
                      ))}
                ...
                {relatedServices.length > 0 && (
                  <div className="mt-8 pt-8 border-t border-slate-200">
                    <h2 className="font-heading font-semibold text-slate-900 mb-4 text-sm uppercase tracking-[0.18em]">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                      {relatedServices.map((s) => (
                        <Link
                          ...
                          className="group rounded-xl border border-slate-200 bg-white p-4 transition-all duration-300 hover:border-primary-500/40 hover:bg-slate-50"
                        >
                          <div className="font-heading font-semibold text-slate-900 text-sm mb-1 group-hover:text-primary-600 transition-colors">
                            {s.title}
                          </div>
                          {s.price ? (
              ...
              <aside className="lg:col-span-4">
                <div className="sticky top-24 space-y-6">
                  ...
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 backdrop-blur-sm">
                    <h3 className="font-heading font-semibold text-slate-900 mb-4 text-sm uppercase tracking-[0.18em]">About the Author</h3>
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <div>
                        <div className="font-heading font-semibold text-slate-900">{post.author.name}</div>
                        <div className="text-xs text-slate-500">{post.author.role}</div>
                      </div>
                    </div>
                    <p className="text-slate-600 text-sm leading-relaxed">Passionate about helping businesses succeed in the digital world through innovative technology and strategic solutions.</p>
                  </div>

                  {/* Newsletter */}
                  <div className="relative overflow-hidden rounded-2xl border border-slate-200 bg-void-100/90 p-6 border-glow">
                    <div className="glow-orb -top-8 -right-8 w-32 h-32 bg-primary-500/[0.18]" />
                    <div className="relative z-10">
                      <h3 className="font-heading font-bold text-slate-900 mb-2">Stay Updated</h3>
                      <p className="text-slate-600 text-sm mb-4">Get the latest articles delivered to your inbox.</p>
                      <form className="space-y-3">
                        <input type="email" placeholder="Your email" className="w-full px-4 py-2.5 rounded-xl bg-white border border-slate-300 text-slate-900 placeholder-slate-500 focus:outline-none focus:border-primary-500/50 text-sm transition-colors" />
                  ...
                  {/* Quick links */}
                  <div className="rounded-2xl border border-slate-200 bg-white p-6 backdrop-blur-sm">
                    <h3 className="font-heading font-semibold text-slate-900 mb-4 text-sm uppercase tracking-[0.18em]">Quick Links</h3>
                    <ul className="space-y-2.5">
                      {[{ label: 'Our Services', href: '/services' }, { label: 'View Portfolio', href: '/portfolio' }, { label: 'Contact Us', href: '/contact' }].map(({ label, href }) => (
                        <li key={href}>
                          <Link href={href} className="text-slate-600 hover:text-primary-600 text-sm transition-colors flex items-center gap-2 group">
        ...
        <section className="relative py-10 bg-void-50">
          ...
          <div className="container-custom">
            <div className="flex items-center gap-3 mb-8">
              ...
              <h2 className="text-xl font-heading font-bold text-slate-900">Related Articles</h2>
            </div>
